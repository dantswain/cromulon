defmodule Cromulon.Discovery.Postgres do
                         ...
                         |> Enum.flat_map(
                           fn(schema) ->
                             ...
                             columns = tables
                                       ...
                                       |> Enum.flat_map(
                                         fn(t) -> describe_columns(source, schema, t) end
                                       )
  ...
  def describe_tables(source = %Source{}, schema = %Node{kind: "postgres schema"}) do
    Enum.flat_map(
      list_tables(source.connection_info, schema.name),
      fn([table_name]) ->
        node_uuid = UUID.generate()
        [
          %Node{
  ...
  def describe_columns(
    ...
    Enum.flat_map(
      list_columns(source.connection_info, schema.name, table.name),
      fn([column_name, column_type]) ->
        node_uuid = UUID.generate()
        [
          %Node{
            name: column_name,
