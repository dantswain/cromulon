defmodule Cromulon.Discovery.Kafka do
  ...
  defp get_partition_sample_messages(topic, partition_id, worker, lookback) do
    ...
    if offset < 0 do
      Logger.warn(
        fn -> "Partition #{topic.name}:#{partition_id} has no messages" end
      )
      []
    else
      topic.name
  ...
  defp with_kafkaex(uris, cb) do
    {:ok, worker} = KafkaEx.create_worker(
      :cromulon_discovery,
      uris: uris
    )
